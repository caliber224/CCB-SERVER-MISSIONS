...
_handle_config = [] execVM "functions\module_monitor\config.sqf";
waitUntil{scriptDone _handle_config};

pvpfw_perf_monitorResultsToRPT = compile preProcessFileLineNumbers "functions\module_monitor\dumpPerformanceArrays.sqf";
...
while{true}do{
	...
	if (pvpfw_monitor_infoOnMap) then{
		_infoMarker setMarkerText format["服务器: fps %1 | 玩家: %2 | 地图中的所有物体: %3",([_currentfps,1] call BIS_fnc_cutDecimals),_currentPlayers,_currentObjectCount];
	};
